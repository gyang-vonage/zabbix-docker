...
update_zbx_config() {
    ...
    update_config_var $ZBX_CONFIG "TLSConnect" "cert"
    update_config_var $ZBX_CONFIG "TLSAccept" "${ZBX_TLSACCEPT}"
    update_config_var $ZBX_CONFIG "TLSCAFile" "zabbix_ca_file"
    update_config_var $ZBX_CONFIG "TLSCRLFile" "${ZBX_TLSCRLFILE}"
    update_config_var $ZBX_CONFIG "TLSServerCertIssuer" "CN=Vonage Signing CA,OU=Vonage Signing CA,O=Vonage,DC=monitoring,DC=org"
    update_config_var $ZBX_CONFIG "TLSServerCertSubject" "CN=zabbix.mon1.mem.dev.use1.vonagenetworks.net,O=Vonage,DC=monitoring,DC=org"

    update_config_var $ZBX_CONFIG "TLSCertFile" "zabbix-proxy-sqlite3.crt"
    ...
    update_config_var $ZBX_CONFIG "TLSKeyFile" "zabbix-proxy-sqlite3.key"
